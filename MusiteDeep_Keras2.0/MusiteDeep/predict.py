...
def main():
    ...
    parser.add_argument('-predict-type', dest='predicttype', type=str,
    ...
    parser.add_argument('-cnn', dest='cnn', type=str,
                        help='name of the CNN model to be trained. \'DeepBind\' and \'MusiteDeep\' are accepted. It '
                             'must be the same as the model used to make predictions.',
                        required=False, default="MusiteDeep")
    ...
    if predicttype == 'general': #prediction for general phosphorylation
        ...
        if("S" in residues or "T" in residues):
               ...
               models=cnn.create_model(testX.shape[2], testX.shape[3])# only to get config
        ...
        if modelprefix is None:
          ...
          testX,testY = convertRawToXY(testfrag.as_matrix(),codingMode=0) 
          predictproba=np.zeros((testX.shape[0],2))
          models=cnn.create_model(2*window + 1, testX.shape[3])# only to get config
          ...
          if(parameters.split("\t")[3]=="kinase-specific"):
               ...
               for ini in range(nclass_ini):
                   for bt in range(nclass):
                        ...
                        predictproba+=models.predict(np.squeeze(testX))
