...
def main():
    ...
    parser.add_argument('-cnn', dest='cnn', type=str,
                        help='name of the CNN model to be trained. \'DeepBind\' and \'MusiteDeep\' are accepted. It '
                             'must be the same model used for pre-training.', required=False, default="MusiteDeep")
    parser.add_argument('-lr', dest='lr', type=float,
                        help='value of the learning rate used by the Adam optimizer when fine-tuning the model.',
    ...
    # CNN model argument
    from methods.multiCNN import DeepBind, MusiteDeep
    # An additional script argument should be added if custom model hyperparameters values are desired; default values
    # are currently used
    cnn_dict = {'DeepBind': DeepBind(), 'MusiteDeep': MusiteDeep()}
    if cnn not in cnn_dict:
        print 'ERROR: Available values for cnn argument are: \'DeepBind\', \'MusiteDeep\'.'
        exit()
    ...
    for ini in range(nclass_init):
        weights = backgroundmodel+'_class'+str(ini)
        print "load weights:" + weights
        background = cnn.create_model(input_row=2*window+1, input_col=21)  # as codingmode=0 (default)
        background.load_weights(weights)
        for bt in range(nclass):
            models=bootStrapping_allneg_continue_val(trainfrag.as_matrix(),valfile=valfrag,
                                                     ...
                                                     transferlayer=transferlayer, cnn=cnn, optimizer=Adam(lr=lr))
