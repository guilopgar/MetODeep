...
from multiCNN import  MusiteDeep
...
def bootStrapping_allneg_continue_val(trainfile,valfile=None,srate=0.8,nb_epoch1=3,nb_epoch2=30,earlystop=None,
                                      maxneg=None,codingMode=0,transferlayer=1,inputweights=None,outputweights=None,
                                      forkinas=False, cnn=MusiteDeep(), optimizer=Adam()): #inputfile:fragments (n*34);srate:selection rate for positive data;nclass:number of class models
  ...
  for I in range(nb_epoch1):
    ...
    for t in range(nclass):
        ...
        if t==0:
            models=cnn.cnn_train(trainX1,trainY1,valX1,valY1,nb_epoch=nb_epoch2,earlystop=earlystop, optimizer=optimizer,
                                 transferlayer=transferlayer,weights=inputweights,forkinas=forkinas,compiletimes=t)
        else:
            models=cnn.cnn_train(trainX1,trainY1,valX1,valY1,nb_epoch=nb_epoch2,earlystop=earlystop, optimizer=optimizer,
                                 transferlayer=transferlayer,weights=inputweights,forkinas=forkinas,compiletimes=t,
